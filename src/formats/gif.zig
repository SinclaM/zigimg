...
pub const GIF = struct {
    ...
    pub fn read(self: *GIF, stream: *Image.Stream) Image.ReadError!Image.Animation.FrameList {
        ...
        const global_color_table_size = std.math.pow(usize, 2, @as(usize, @intCast(self.header.flags.global_color_table_size)) + 1);
    ...
    fn readImageDescriptorAndData(self: *GIF, context: *ReaderContext) Image.ReadError!void {
        if (context.current_frame_data) |current_frame_data| {
            ...
            const local_color_table_size = std.math.pow(usize, 2, @as(usize, @intCast(sub_image.image_descriptor.flags.local_color_table_size)) + 1);
    ...
    fn findBestPixelFormat(self: *const GIF) PixelFormat {
        ...
        if (self.header.flags.use_global_color_table) {
            total_color_count = std.math.pow(usize, 2, @as(usize, @intCast(self.header.flags.global_color_table_size)) + 1);
        }
        ...
        for (self.frames.items) |frame| {
            ...
            for (frame.sub_images.items) |sub_image| {
                if (sub_image.image_descriptor.flags.has_local_color_table) {
                    color_per_frame += std.math.pow(usize, 2, @as(usize, @intCast(sub_image.image_descriptor.flags.local_color_table_size)) + 1);
                }
            }
